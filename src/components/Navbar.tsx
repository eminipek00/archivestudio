...
const Navbar = ({ onSearch }: NavbarProps) => {
  ...
  const languages: { code: Language; name: string }[] = [
    { code: 'tr', name: 'Türkçe' }, { code: 'en', name: 'English' }, { code: 'az', name: 'Azərbaycan' }, { code: 'es', name: 'Español' }, { code: 'fr', name: 'Français' }, { code: 'de', name: 'Deutsch' }, { code: 'ar', name: 'العربية' }, { code: 'ru', name: 'Русский' }, { code: 'zh', name: '中文' }, { code: 'ja', name: '日本語' }, { code: 'pt', name: 'Português' },
  ];
  ...
  return (
    ...
    <nav className="fixed top-0 left-0 right-0 z-[5000] py-3 md:py-4 bg-black border-b border-white/5 shadow-2xl">
        ...
        {isMobileSearchActive ? (
          <div className="flex-1 flex items-center gap-2 animate-in slide-in-from-left-4 duration-300 relative">
            ...
            <div className="flex-1 relative">
              ...
              <input autoFocus type="text" value={localSearch} onChange={handleSearchChange} placeholder={t('searchPlaceholder')} className="w-full bg-[#0a0a0a] border border-white/10 rounded-xl py-2.5 pl-10 pr-10 text-[11px] font-black uppercase tracking-widest text-white outline-none focus:border-primary/30 shadow-inner" />
              {localSearch && (
                <button onClick={clearSearch} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 bg-white/5 hover:bg-white/10 rounded-lg transition-all"><X size={12} className="text-white/40" /></button>
              )}
            </div>
              ...
              <div className="absolute top-[120%] left-0 right-0 bg-[#0a0a0a] border border-white/10 rounded-2xl p-2 shadow-2xl animate-in slide-in-from-top-2 duration-200 z-[6000]">
                {searchResults.map(asset => (
                  <button key={asset.id} onClick={() => { setIsMobileSearchActive(false); router.push(`/?asset=${asset.id}`); }} className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-all text-left group">
                    <div className="w-10 h-10 rounded-lg overflow-hidden border border-white/10 shrink-0"><img src={asset.cover_url || '/logo.png'} alt="" className="w-full h-full object-cover" /></div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-[6000] bg-black/80 backdrop-blur-xl border-t border-border-custom px-4 py-3 flex items-center justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
        <Link href="/" onClick={() => { closeAllMenus(); setIsMobileSearchActive(false); }} className="p-2 text-white/40 hover:text-primary transition-all"><Home size={22} /></Link>
        <button onClick={handleMobileSearchToggle} className={`p-2 transition-all ${isMobileSearchActive ? 'text-primary' : 'text-white/40'}`}><Search size={22} /></button>
        <button onClick={() => { closeAllMenus(); setIsMobileSearchActive(false); router.push('/upload'); }} className="p-3 bg-primary text-white rounded-2xl shadow-lg shadow-primary/30 -mt-8 border-4 border-black active:scale-95 transition-all"><Upload size={22} /></button>
        <div className="relative">
            <button onClick={() => { setShowNotifMenu(!showNotifMenu); setShowUserMenu(false); setShowLangMenu(false); setIsMobileSearchActive(false); }} className={`p-2 transition-all relative ${showNotifMenu ? 'text-primary' : 'text-white/40'}`}><Bell size={22} />{unreadCount > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-primary rounded-full border border-black" />}</button>
            {showNotifMenu && (
        ...
        <div className="relative">
          <button onClick={() => { setShowUserMenu(!showUserMenu); setShowNotifMenu(false); setShowLangMenu(false); setIsMobileSearchActive(false); }} className="p-2 text-white/40 hover:text-primary transition-all">
            {profile?.avatar_url ? ( <div className={`w-7 h-7 rounded-lg overflow-hidden border transition-all ${showUserMenu ? 'border-primary' : 'border-white/20'}`}><img src={profile.avatar_url} alt="P" className="w-full h-full object-cover" /></div> ) : ( user ? <Settings size={22} className={showUserMenu ? 'text-primary' : ''} /> : <LogIn size={22} /> )}
          </button>
          {showUserMenu && (
            <div className="fixed bottom-[80px] left-4 right-4 bg-[#0a0a0a] border border-border-custom rounded-[2rem] p-4 shadow-2xl animate-in slide-in-from-bottom-2 duration-300 z-[7000]">
                <div className="flex items-center gap-4 p-4 border-b border-white/5 mb-2 text-left">
                  <div className="w-12 h-12 rounded-2xl overflow-hidden border border-white/10 bg-[#050505]"><img src={profile?.avatar_url || '/logo.png'} alt="P" className="w-full h-full object-cover" /></div>
                  <div className="flex flex-col"><span className="text-xs font-black text-white uppercase italic">@{profile?.username || 'user'}</span></div>
                </div>
                <div className="grid grid-cols-1 gap-1 text-left">
                  <Link href="/profile" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[10px] font-black text-white/60 hover:text-white hover:bg-white/5 transition-all"><User size={18} /> {t('profile').toUpperCase()}</Link>
                  <Link href="/support" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[10px] font-black text-white/60 hover:text-white hover:bg-white/5 transition-all"><MessageSquare size={18} /> {t('supportCenter').toUpperCase()}</Link>
                  <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[10px] font-black text-red-400 hover:bg-red-500/10 transition-all text-left"><LogOut size={18} /> {t('logout').toUpperCase()}</button>
                </div>
            </div>
          )}
        </div>
