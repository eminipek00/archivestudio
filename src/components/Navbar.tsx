  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[5000] py-4 bg-[#050505] border-b border-border-custom shadow-2xl">
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between gap-8">
        ...
        {/* SEARCH BAR - TASARIMI BOZMAYAN YENİ YERİ */}
        {onSearch && (
          <div className="hidden md:flex flex-1 max-w-md relative group">
            ...
            <input type="text" value={localSearch} onChange={handleSearchChange} placeholder={t('searchPlaceholder')} className="w-full bg-[#111] border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-[10px] font-black uppercase tracking-widest text-white outline-none focus:border-primary/50 transition-all" />
          </div>
        )}
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          {/* LANGUAGE SELECTOR - SIFIR ŞEFFAFLIK */}
          <div className="relative">
            <button onClick={() => { setShowLangMenu(!showLangMenu); setShowUserMenu(false); }} className="px-5 py-2.5 rounded-xl bg-[#111] border border-white/10 text-white/60 hover:text-white hover:bg-white/5 transition-all flex items-center gap-3">
          ...
          {user ? (
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <button onClick={() => { setShowUserMenu(!showUserMenu); setShowLangMenu(false); }} className="w-11 h-11 rounded-xl overflow-hidden border-2 border-white/10 hover:border-primary transition-all p-0.5 bg-[#111]">
                  <img src={profile?.avatar_url || '/logo.png'} alt="P" className="w-full h-full object-cover rounded-lg" />
                </button>

                {showUserMenu && (
                  <div className="absolute top-full right-0 mt-3 w-64 bg-[#0a0a0a] border border-border-custom rounded-[2.5rem] p-4 shadow-2xl animate-in slide-in-from-top-4 duration-300 z-[6000]">
                    <div className="flex items-center gap-4 p-4 border-b border-white/5 mb-2">
                      <div className="w-12 h-12 rounded-2xl overflow-hidden shrink-0 border border-white/10 bg-[#111]">
