...
import { Globe, LogIn, UserPlus, Upload, LogOut, Settings, Search, Edit3, Check, Maximize2, Palette, MessageSquare, Bell, Home, User, LifeBuoy } from 'lucide-react';
  ...
  return (
    ...
    <nav className="fixed top-0 left-0 right-0 z-[5000] py-3 md:py-4 bg-black border-b border-border-custom shadow-2xl">
      <div className="max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between gap-2 md:gap-8">
        ...
        <div className="flex items-center gap-2 md:gap-6 shrink-0 text-left">
            ...
            <div className="md:hidden relative">
              <button onClick={() => { setShowLangMenu(!showLangMenu); setShowUserMenu(false); setShowNotifMenu(false); }} className="p-2 text-white/40 hover:text-primary transition-all">
                <Globe size={20} />
              </button>
              {showLangMenu && (
                <div className="absolute top-full left-0 mt-3 w-40 bg-[#0a0a0a] border border-border-custom rounded-2xl p-2 shadow-2xl animate-in zoom-in-95 duration-200 z-[6000]">
                  <div className="grid grid-cols-1 gap-1 max-h-[300px] overflow-y-auto no-scrollbar">
                    {languages.map((lang) => (
                      <button key={lang.code} onClick={() => { setLanguage(lang.code); setShowLangMenu(false); }} className={`flex items-center justify-start px-3 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${language === lang.code ? 'bg-primary text-white' : 'text-white/40'}`}>
                        {lang.name}
                      </button>
                    ))}
                  </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-[6000] bg-black/80 backdrop-blur-xl border-t border-border-custom px-4 py-3 flex items-center justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
        ...
        <div className="relative">
          <button onClick={() => { setShowUserMenu(!showUserMenu); setShowNotifMenu(false); setShowLangMenu(false); }} className="p-2 text-white/40 hover:text-primary transition-all">
            {profile?.avatar_url ? (
            ...
            <div className="fixed bottom-[80px] left-4 right-4 bg-[#0a0a0a] border border-border-custom rounded-[2rem] p-4 shadow-2xl animate-in slide-in-from-bottom-2 duration-300 z-[7000]">
                ...
                <div className="grid grid-cols-1 gap-1">
                  ...
                  <Link href="/support" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-3.5 rounded-2xl text-[10px] font-black text-white/60 hover:text-white hover:bg-white/5 transition-all">
                    <LifeBuoy size={18} /> {t('supportCenter').toUpperCase()}
                  </Link>
