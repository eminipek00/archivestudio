...
export const sendVerificationEmail = async (email: string, token: string) => {
  try {
    const data = await resend.emails.send({
      from: 'Sytex Archive <noreply@sytexarchive.com>',
      to: [email],
      subject: 'Sytex Archive Hesabını Doğrula',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #020617; color: white; border-radius: 20px;">
          <h1 style="color: #3b82f6; text-align: center;">SYTEX ARCHIVE</h1>
          <p style="font-size: 18px; text-align: center;">Merhaba!</p>
          <p style="text-align: center;">Sytex Archive hesabınızı doğrulamak için aşağıdaki kodu kullanın veya butona tıklayın.</p>
