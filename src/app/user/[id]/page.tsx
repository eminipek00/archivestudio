...
import { User, Database, Heart, Trophy, ChevronLeft, Loader2, FolderOpen } from "lucide-react";
...
const UserProfilePage = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (profileData) {
          ...
          if (assetData) {
              setAssets(assetData);
              setTotalLikes(Math.floor(assetData.length * 15.5)); // Beğeni simülasyonu (Altyapı hazır)
          }
      }
      setLoading(false);
    };
  ...
  if (!profile) return <div className="h-screen w-full bg-black flex items-center justify-center text-white font-black">USER NOT FOUND LO!</div>;
      ...
      <main className="flex-grow overflow-y-auto px-4 py-8 custom-scrollbar">
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="bg-card border border-border-custom p-8 md:p-12 rounded-[3.5rem] shadow-2xl relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
            ...
            <div className="w-40 h-40 md:w-56 md:h-56 rounded-[4rem] overflow-hidden border-8 border-background shadow-2xl bg-[#111] shrink-0 transform -rotate-3 hover:rotate-0 transition-transform duration-500">
                <img src={profile.avatar_url || '/logo.png'} alt="Profile" className="w-full h-full object-cover" />
            </div>

            <div className="flex-1 text-center md:text-left space-y-4">
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                    <h1 className="text-4xl md:text-6xl font-[1000] uppercase italic tracking-tighter text-white drop-shadow-2xl">{profile.full_name || 'Sytex Editor'}</h1>
                    {isAdmin && <span className="text-[10px] font-black bg-primary text-white px-4 py-2 rounded-2xl uppercase italic tracking-widest shadow-xl shadow-primary/20">MASTER ARCHIVIST</span>}
                </div>
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 pt-4">
                    <div className="px-6 py-3 bg-white/5 border border-white/5 rounded-3xl flex items-center gap-4 hover:bg-white/10 transition-all">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-[8px] font-black text-white/30 uppercase tracking-widest">UPLOADED</span>
                        </div>
                    </div>
                    <div className="px-6 py-3 bg-white/5 border border-white/5 rounded-3xl flex items-center gap-4 hover:bg-white/10 transition-all">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-[8px] font-black text-white/30 uppercase tracking-widest">TOTAL LIKES</span>
                        </div>
                    </div>
                </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4 px-4">
                ...
                <h2 className="text-xl font-black uppercase italic tracking-tighter text-white/40">VİTRİN / ASSETS</h2>
            ...
            {assets.length === 0 ? (
                <div className="py-20 flex flex-col items-center justify-center text-white/20">
                    ...
                    <span className="text-xs font-black uppercase tracking-widest">Henüz bir şey yüklememiş lo!</span>
                </div>
            ) : (
