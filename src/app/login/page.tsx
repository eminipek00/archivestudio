...
const LoginPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Beni Hatırla mantığı: Supabase varsayılan olarak oturumu hatırlar.
      // Özel süre ayarları Supabase dashboard üzerinden yapılır.
    ...
    <div className="min-h-screen w-full flex items-center justify-center p-4 bg-background">
      <div className="max-w-[400px] w-full space-y-8 glass-panel p-8 md:p-10 rounded-[2rem] shadow-2xl">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-1 text-xs font-bold uppercase tracking-widest">Giriş Yap</p>
        </div>
          ...
          <div className="flex items-center justify-between px-1">
            <label className="flex items-center gap-2 cursor-pointer group">
              <input 
                ...
                className="w-4 h-4 rounded border-border-custom text-primary focus:ring-primary bg-muted/50"
              />
